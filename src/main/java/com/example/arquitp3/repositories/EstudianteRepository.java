...
public interface EstudianteRepository extends JpaRepository<Estudiante, Long> {
    ...
    @Query("SELECT new com.example.arquitp3.model.Estudiante(e.libreta_universitaria,e.nombre,e.apellido,e.edad,e.genero,e.ciudad_residencia)" +
    ...
    Estudiante findByLibretaUniversitaria(long libreta_universitaria);

    @Query("SELECT new com.example.arquitp3.model.Estudiante(e.libreta_universitaria, e.nombre, e.apellido, e.edad, e.genero, e.ciudad_residencia) " +
            "FROM Estudiante e WHERE e.genero = :genero")
    List<Estudiante>findAllByGenero(String genero);

    @Query("SELECT new com.example.arquitp3.dtos.EstudianteDto(e.libreta_universitaria, e.nombre, e.apellido, e.edad, e.ciudad_residencia, cc.fecha_inscripcion) " +
            "FROM Estudiante e " +
            "JOIN e.carreras cc " +
